	...
	return one()
		.then((res) => {
			...
			return Promise.sleep(
				duration < throttle ? Math.min(throttle - duration, delay) : delay,
				res,
			);
		})
		.then((res) => {
				...
				return new Promise((resolve, reject) => {
					...
					values.forEach((res, index) => {
						logListener({
							step: 1,
							message:
								"Extracting " + modSelection[index].displayName + " into final zip",
						});
							...
							.then((zip) => {
								...
								if (success === modSelection.length) {
									logListener({
										step: 2,
										message:
											"Inserting pack.png and pack.mcmeta into final zip",
									});
									...
									fetch(PATH_PACK_PNG)
										...
										.then((packImageBlob) => {
											finalZip.file("pack.png", packImageBlob, {
												blob: true,
											});
											finalZip.file(
												"pack.mcmeta",
