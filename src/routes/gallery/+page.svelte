<script lang="ts">
	...
	function search(_origin?: string) {
		...
		const params = gallerySearch.getAllParams();
		const params_obj = [...params.entries()].reduce(
			(acc, cur) => {
				acc[cur[0]] = cur[1];
				return acc;
			},
	...
	$: styles_gallery_text = Object.entries(font_size_not_done)
		.map(([c, e]) => `--not-done-${c}: ${e}; `)
		.join(" ");
