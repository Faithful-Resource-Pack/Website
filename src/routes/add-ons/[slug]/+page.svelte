<script lang="ts">
	...
	let text_date_last_updated: boolean | string =
		!!data.lastUpdated &&
		new Date(data.lastUpdated)?.toLocaleDateString("pt-br").split("/").reverse().join("-");
	let text_last_updated =
		$t("add-ons.post.last_updated") + " " + (text_date_last_updated || $t("common.unknown"));
	...
	let screenList = data.screenshots.map((e: string, i: number) => ({
		image: e,
		alt: "Screenshot " + i,
	}));
...
<div class="container">
	...
	<div class="addon-details">
				...
				<div class="card card-body">
					{#if data.information.optifine == true}
						<div class="addon-info">
							<img
								class="addon-info-img"
								src="/images/icon/optifine.png"
								loading="lazy"
								alt="requires optifine"
							/>
							<p>
								{$t("add-ons.post.requirement")}
								<a href="https://optifine.net/downloads">OptiFine</a>
							</p>
						</div>
					{/if}
				...
				<div>
					{#each data.authors as author}
						<div class="addon-author">
							{#if author.uuid}
								<img
									class="addon-author-head"
									src="https://visage.surgeplay.com/head/128/{author.uuid}"
									loading="lazy"
									alt="{author.username}'s Skin"
								/>
							{:else}
								<img
									class="addon-author-head"
									src="https://visage.surgeplay.com/head/128/{[
										'X-Steve',
										'X-Alex',
										'X-Ari',
										'X-Efe',
										...
										<a href={media.link} target="_blank" rel="noreferrer">
											...
											{:else}
												<img
													width="20"
													height="20"
													src={media_icons[media.type].src}
													alt={media.type}
												/>
											{/if}
										</a>
		...
		<div class="addon-details-right">
			<img
				class="addon-header"
				src="https://database.faithfulpack.net/images/addons/{data.slug}/header"
			...
			{#if screenList.length > 0}
				...
				<svelte:component
					this={ImagePreviewer}
					bind:this={previewer}
					bind:displayed={previewerDisplayed}
					images={screenList}
				/>
			{/if}
