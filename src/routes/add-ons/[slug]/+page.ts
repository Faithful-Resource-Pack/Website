...
export const load: PageLoad = async function ({ fetch, params }) {
	const addonRes = await fetch(`https://api.faithfulpack.net/v2/addons/${params.slug}`).catch(
		(err) => {
			console.error("addonData", err);
			throw err;
		},
	);
	const addonData = await addonRes.json();

	if (addonData.name == "NotFound") throw error(404, "Not found");

	const fileRes = await fetch(
		`https://api.faithfulpack.net/v2/addons/${params.slug}/files/downloads`,
	).catch((err) => {
	...
	const headerRes = await fetch(
		`https://api.faithfulpack.net/v2/addons/${params.slug}/files/header`,
	).catch((err) => {
	...
	const AuthorRes = await fetch(
		`https://api.faithfulpack.net/v2/addons/${params.slug}/authors`,
	).catch((err) => {
	...
	const screenRes = await fetch(
		`https://api.faithfulpack.net/v2/addons/${params.slug}/files/screenshots`,
	).catch((err) => {
