...
const shortMd = function (md: string, length = 157) {
	...
	renderer.text = function (t: string) {
		return t;
	};
	...
	const desc_txt = DOMPurify.sanitize(
		marked.parse(md, {
			renderer,
		}),
	)
	...
	let short = `${
		desc_txt.split(" ").reduce(
			([acc, done]: [string, boolean], cur: string): [string, boolean] => {
	...
	if (short.length < desc_txt.length && short[short.length - 1] != ".") short += "...";
...
function parseMarkdownList(markdownList: string): ListItem[] {
	...
	const groupedLines = indentedLines
		.map((i) => (i[0] === minIndent ? [i[1]] : [, i[1]]))
		.reduce(
			(acc, cur) => {
