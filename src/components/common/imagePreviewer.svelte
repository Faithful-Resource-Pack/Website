...
{#if displayed}
    <div transition:fade="{{duration: 200}}" id="fullscreen-preview" on:keydown={handleEscape} on:click|self={hide}>
        <div id="hide" class="nav" on:click={hide} on:keydown={handleKeyDown('x', hide)}>
            <Fa icon={faTimes} />
        </div>
        <div id="current-content" bind:this={contentElement} transition:scale on:click|self={hide} on:keydown={handleEscape}>
            {#if multiple}
...
{#if multiple}
    <div id="inline-list" class="res-grid-4">
        {#each imageStringList as image,i}
            <div class="preview card" on:click={show(i)} on:keydown={() => {}}><img src={image.image} alt={image.alt}></div>
        {/each }
    </div>
{/if}
