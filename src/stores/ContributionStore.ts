...
let _contributions = readable({
    ...
    .then(res => res.json())
    .then(json => {
        let reduced = json.filter((contribution: Contribution) => contribution.pack && contribution.texture)
    ...
    .catch((err: Error) => {set({ data: undefined, error: err })})
})
