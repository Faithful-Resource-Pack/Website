self.Compliance = {
  ...
  cache: false
    /*
       ...
       Compliance site, I think it's because there are a lot of images.
       - Offroaders123
    */
}
self.addEventListener("activate",event => {
  event.waitUntil(caches.keys().then(versions => Promise.all(versions.map(cache => {
    if (cache !== Compliance.version) return caches.delete(cache);
  }))));
...
self.addEventListener("fetch",event => {
  event.respondWith(caches.match(event.request).then(response => {
    return response || fetch(event.request).then(async response => {
      if (Compliance.cache) caches.open(Compliance.version).then(cache => cache.put(event.request,response));
